...
create or replace noneditionable procedure ddpay_sp (i_id in number , o_result out boolean)
as
  v_pledge_counter number;
begin
  select count(*) into v_pledge_counter
    from dd_pledge where iddonor = i_id and idstatus = 10 and paymonths > 0;
  if v_pledge_counter > 0 then
    o_result := true;
  else
    o_result := false;
  end if;
...
create or replace noneditionable procedure ddckpay_sp (i_payment in number , i_pledge_id in number)
as
  v_monthly_pay number;
begin
  select pledgeamt / paymonths into v_monthly_pay
    from dd_pledge where idpledge = i_pledge_id and paymonths > 0;
    if i_payment = v_monthly_pay then
      dbms_output.put_line('Payment is the correct amount');
    else
      raise_application_error(-20050, 'Incorrect payment amount - planned payment = ' || v_monthly_pay);
    end if;
exception
